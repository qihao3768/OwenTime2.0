...
//        paint.isElegantTextHeight = true
    ...
    constructor(context: Context, attr: AttributeSet?) : super(context, attr) {
        val typeArray = context.obtainStyledAttributes(attr, R.styleable.ExpandTextView)
        if (typeArray == null) {
            return
        }
        val num = typeArray.indexCount
...
//        log.d("w/h:$w/$h,ow/od:$oldw/$oldh")
     ...
     * 获取自己测量后的高度
     */
    private fun calHeight(width: Int): Float {
        ...
        val staticLayout = getStaticLayout(currentText, contentPaint, width)
        ...
        if (lineCount <= maxLine) {
            ...
            for (i in 0 until lineCount) {
                //获取当前行内容
                val currentLinStr = currentText.substring(
                    staticLayout.getLineStart(i),
                    staticLayout.getLineEnd(i)
                )
                val currentLinStaticLayout = getStaticLayout(currentLinStr, contentPaint, width)
                val currentLineHeight = getStaticLayoutHeight(currentLinStaticLayout)
...
//                log.d("计算展开状态下的高度")
                ...
                for (i in 0 until lineCount) {
                    ...
                    if (i == lineCount - 1) {
                        //特殊展示的内容数据
                        val shrinkStaticLayout = getStaticLayout(shrinkText, specialPaint, width)
                        val shrinkWidth = getShrinkWidth(shrinkStaticLayout)
                        //主内容最后一行的宽度
                        val currentLinStrWidth = currentLinStaticLayout.getLineWidth(0)
...
//                            log.d("需要另起一行")
                            ...
                            height += getShrinkHeight(shrinkStaticLayout) + bottomMargin
                        } else {
                            //取最高的那个
                            val max = max(
...
//                            log.d("取较高一个高度:$max")
                        ...
                        //普通行
                        val currentLineHeight = getStaticLayoutHeight(currentLinStaticLayout)
                        height += currentLineHeight + bottomMargin
...
//                        log.d("取较大高度:$max，该行内容:$currentLinStr")
                        ...
                        //普通行
                        val normalHeight = getStaticLayoutHeight(currentLinStaticLayout)
...
//                log.d("计算收起状态总显示高度:$height")
    ...
    private fun notifyExpandListener(isExpand: Boolean) {
     ...
     * 获取开始的索引
     */
    private fun getStartPainIndex(): Float {
        return if (isRtl()) {
            width.toFloat()
        } else {
            0f
...
//            log.e("当前需要绘制的文本内容为空")
        ...
        if (lineCount <= maxLine) {
            ...
            for (i in 0 until lineCount) {
                ...
                val currentLineStaticLayout = getStaticLayout(currentLineStr, contentPaint, width)
                val currentLineHeight = getStaticLayoutHeight(currentLineStaticLayout)
                drawText(
                    canvas,
            ...
            if (isExpand) {
                //当前状态为展开状态，绘制展开状态内容
                val shrinkStaticLayout = getStaticLayout(shrinkText, specialPaint, width)
                val shrinkStaticLayoutWidth = getShrinkWidth(shrinkStaticLayout)
                val shrinkStaticLayoutHeight = getShrinkHeight(shrinkStaticLayout)
                for (i in 0 until lineCount) {
                    //获取当前行内容
                    val currentLineStr = currentText.substring(
                        staticLayout.getLineStart(i),
                        staticLayout.getLineEnd(i)
                    )
                    val currentLineStaticLayout =
                    ...
                    if (i == lineCount - 1) {
                        ...
                        if ((currentLineWidth + specialLeftMargin + shrinkStaticLayoutWidth) > width) {
                            //展开内容需要换行处理
                            //绘制主内容
                            val currentHeight = getStaticLayoutHeight(currentLineStaticLayout)
...
//                            log.d("当前最后一行最底部距离:${height + shrinkStaticLayoutHeight + bottomMargin}")
                ...
                //当前为收起状态，绘制收起状态内容
                //收起状态多了前缀，前缀需要特殊处理
                val expandStaticLayout = getStaticLayout(expandText, specialPaint, width)
                val expandStaticLayoutHeight = getExpandHeight(expandStaticLayout)
                val expandStaticLayoutWidth = getExpandWidth(expandStaticLayout)
                var height = 0f
                for (i in 0 until lineCount) {
                    //获取当前行内容
                    val currentLineStr = currentText.substring(
                    ...
                    if (i == maxLine - 1) {
                        //在最大一行特殊处理
                        //需要计算前缀的内容信息
                        val expandPrifixStaticLayout =
                            getStaticLayout(expandPrifix, contentPaint, width)
                        val expandPrifixStaticLayoutWidth = expandPrifixStaticLayout.getLineWidth(0)
                        ...
                        //临时的一个高度，表达当前主内容的y值
                        val currentHeight = getStaticLayoutHeight(currentLineStaticLayout)
                        val currentLineTmpHeight = height + currentHeight
                        if (true) {
                            //最后一行文本的宽度
...
//                            log.d("缩减完毕:$lastLineWidth")
                            ...
                            for (i in currentLineStr) {
                                ...
                                val currentStringStaticLayout =
                                    getStaticLayout(currentString, contentPaint, width)
                                val currentStringStaticLayoutWidth =
...
//        canvas.drawTextRun(text,0,text.length,0,text.length,x,y,false,paint)
     ...
     */
    private fun getDealStr(text: String? = null): String {
...
//        dest = m.replaceAll("")
    ...
    private fun getDrawTextY(paint: Paint, y: Int): Int {
...
//        var dealText = m.replaceAll("")
        val dealText = getTmpDealText(text)
        val staticLayout =
            StaticLayout(
                dealText,
                0,
...
//        var staticLayout = StaticLayout.Builder.obtain(text, 0, text.length, paint, width).build()
    ...
    private fun getStaticLayoutHeight(staticLayout: StaticLayout): Int {
